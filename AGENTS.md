...
- **`tuned` (`manifests/init.pp:40-95`)** — Public entry class. Parameters and
  their defaults (`init.pp:41-54`):
  ...
  - `$package_ensure` (`String`) — the ensure state of the `tuned` package.
    ...
    (`init.pp:54`). This is the module's only SIMP `simp_options` seam.

  Resources declared (`init.pp:57-94`), all keyed off the local
  `$ktune_name = 'tuned'` (`init.pp:57`):
  ...
  - `file { '/etc/sysconfig/ktune' }` — mode `0640`, content from
    ...
    directly, but the service `require`s it (`init.pp:67-72`).
  - `file { '/etc/sysctl.ktune' }` — `ensure => present`, mode `0640`, empty
    (managed for existence only) (`init.pp:74-79`).
  - `package { 'tuned' }` at `$package_ensure` (`init.pp:81-83`).
  - `service { 'tuned' }` — `ensure => running`, `enable => true`,
    `hasrestart`/`hasstatus => true`, `require => [Package['tuned'],
    File['/etc/sysconfig/ktune']]` (`init.pp:85-94`).
...
It is a simple `Enum` of the four legal elevator values (`types/ioschedule.pp:1`).
...
- **Only `/etc/tuned.conf` notifies the service.** A change to
  ...
  is a `require` of the service, not a `notify` (`init.pp:64,72,90-93`). Editing
...
- **`/etc/sysctl.ktune` is always created empty**, regardless of
  `$use_sysctl_post` (`init.pp:74-79`). The `$use_sysctl_post` flag only controls
  whether `ktune` is told to *read* it (`ktune.erb:17-21`).
...
- **`$elevator_tune_devs` is interpolated into a shell glob** —
  `/sys/block/{hd,sd,cciss}*/queue/scheduler` (`ktune.erb:32`). Values are joined
...
- **The `tuned`/`ktune` split is historical.** On EL7-era systems `tuned` and
  ...
  all just `'tuned'` (`init.pp:57`). The `ktune` naming survives only in the
...
- `simp/simplib` `>= 4.9.0 < 5.0.0` (provides the `simplib::lookup` function used
  for the `simp_options::package_ensure` seam) (`metadata.json:15-18`).
...
Runtime requirement (from `metadata.json` `requirements`, `metadata.json:64-68`):
...
matches: `['>= 7', '< 9']` (`Gemfile:23`), and the Puppet gem is pulled in only
via `gem 'puppet', puppet_version` (`Gemfile:29`).

Supported OS matrix (from `metadata.json`, `metadata.json:24-63`): CentOS 7/8/9;
...
- `spec/spec_helper.rb` — unit test bootstrap (`spec_helper.rb:11` requires
...
- `.github/workflows/pr_tests.yml` runs the **six standard jobs only**:
  ...
  (`pr_tests.yml:32-131`). It uses the older-style global
  `env: PUPPET_VERSION: '~> 7'` (`pr_tests.yml:29-30`).
...
tested Puppet range is `>= 7 < 9` (`Gemfile:23`).

## Conventions

- Preserve the `@summary`-style header and `@param` puppet-strings docstrings on
  the class (`init.pp:1-39`) — they drive `REFERENCE.md`. Regenerate
...
- Route SIMP feature toggles through
  ...
  `$package_ensure` does (`init.pp:54`).
